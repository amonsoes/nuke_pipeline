...
class Evaluator(object):
    def __init__(self, model, opt, unk_model=None, phon_model=None):
        self.opt = opt
        self.model = model
    ...
    def eval(self, data_iter, pred_file=None):
        ...
        for i, batch in enumerate(val_iter):
            ...
            if self.phon_model != None:
                preds = lib.metric.handle_unk_with_phon(src, src_sent_words, preds, self.unk_model, self.phon_model, unk_file)
            else:
        ...
        if self.opt.interactive:
            ...
            return valid_loss, results['f1'], all_preds
