...
class DataLoader(object):
    def __init__(self, tweets, vocab, mappings, opt):
        self.opt = opt
        if self.opt.input == 'phonetic':
            self.ipa_transliterator = PhonTransliterator()
    ...
    def load_data(self, tweets):
        ...
        for tweet in tweets:
            ...
            if(self.opt.input == 'spelling') or (self.opt.input == 'phonetic'): #character model word2word corrections
                    ...
                    if iword and oword and iword.isalnum() and oword.isalnum():
                        if iword == oword and len(iword)>1 and len(oword)>1 and not any(c.isdigit() for c in iword) and  not any(c.isdigit() for c in oword):
                            if random.random() > 0.9 and not self.opt.data_augm:
                                continue
                        if self.opt.input == 'phonetic' and  not self.opt.phonetic_data:
                            iword = self.ipa_transliterator(iword)
                        ...
                        if(self.opt.data_augm):
                            if random.random() > (1 - self.opt.noise_ratio) and self.opt.input != 'phonetic':
                                if iword == oword and len(iword)>1 and len(oword)>1 and not any(c.isdigit() for c in iword) and  not any(c.isdigit() for c in oword):
    ...
    def add_noise(self, word):
        """
            There are 8 kinds of errors we can introduce for data aug:
        ...
        if op in [i for i in range(12, 22)]:
            if word.endswith('ing'):
                return word[:-1]
...
def create_datasets(opt):
    ...
    if val:
        val_data, val_vocab, mappings = create_data(val, opt=opt, vocab=vocab, mappings=mappings)
    else: 
...
def read_file(fn, valsplit=None):
    ...
    if(valsplit):
        ...
        split_number = int(len(tweets)*valsplit)
        train = tweets[:split_number]
        val = tweets[split_number:]
        return train, val
    return tweets, []
